...
export default function Book() {
    const {id} =  useParams();
    const books = featuredBooksData.map((data) =>{
        if (data.id == id) {
            return data;
        }
        return null;
    ...
    return (
        <section className="px-10 lg:px-24 xl:px-36 overflow-hidden">
            <div className='headear-container max-w-[1880px] w-[90%] my-0 mx-auto h-full flex items-center justify-center relative py-2 md:py-10 lg:py-[20px] '>
                ...
                {
                    sellingBooksData.map(({ img, name, writer, desc, price, shopbtnLink }) => {
                        return (
                            <div className="header-wrapper container flex flex-col md:flex-row mt-0 items-center gap-1 md:gap-[10px] xl:gap-[20px] mx-auto h-full w-full">
                                ...
                                <div className="Header-right max-w-[1024px] w-full ">
                                    <h1 className=" text-2xl md:text-3xl xl:text-5xl xl:leading-[50px] text-[#222222] font-[prata,serif] z-10">{name}</h1>
                                    <img src={victor} alt='victor img' className='w-12' />
                                    <h5 className='font-[prata,serif] text-lg sm:text-xl md:text-2xl mt-7 '>{writer}</h5>
                                    <p /*dangerouslySetInnerHTML={{__html:info}}*/ className="text-sm md:text-base lg:text-xl md:leading-[25px] lg:leading-[200%] tracking-[4%] font-['Plus Jakarta Sans', sans-serif] mt-1 md:mt-3 mb-4 lg:mb-7 ">{desc}</p>
                                    <h5 className='text-2xl my-3'>{price}</h5>
                                    <Link to={shopbtnLink}>
